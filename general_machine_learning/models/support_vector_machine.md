...
### D-2.0. Summary: Mind map from optimization to KKT
...
### D-2.1. Constrained optimization problem
...
#### D-2.1.1. Geometric Understanding on constraints
...
#### D-2.1.2. Another view of the constrained optimization problem
...
### D-2.2. Convert the constrained optimization to unconstrained optimization
...
#### D-2.2.1. Design principle for the unconstrained problem.
...
#### D-2.2.2. Validate a $\theta_P(x)$ constructed with Lagrangian function
...
$$\theta_P(\boldsymbol{x}):=\max_{\boldsymbol{\alpha},~\boldsymbol{\beta};~\beta_j\geq0}L(\boldsymbol{x},\boldsymbol{\alpha},\boldsymbol{\beta}) \tag{D-2.2.2-1}$$

$$L(\boldsymbol{x},\boldsymbol{\alpha},\boldsymbol{\beta})=f(\boldsymbol{x})+\sum_{i=1}^m\alpha_i h_i(\boldsymbol{x})+\sum_{j=1}^n\beta_j g_j(\boldsymbol{x}) \tag{D-2.2.2-2}$$
...
$$
- In sum, for $\theta_P(x)$
$$\theta_P(x)=\left\{ \begin{array}{ll} f(x) & x\in\Phi\\ +\infty & x\notin\Phi \end{array} \right. \tag{D-2.2.2-3}$$
- Then:
$${\text{minimize }} f(x) {\text{ for }} x\in\Phi \Leftrightarrow {\text{minimize }} \theta_P(\boldsymbol{x}) \tag{D-2.2.2-4}$$
...
#### D-2.3.0. Summary: Why to introduce dual problem (To be Refined)
...
#### D-2.3.1. Lagrange dual function and Lagrange dual problem

Lagrange dual function:

$$\theta_D(\boldsymbol{\alpha},\boldsymbol{\beta}) = \underset{x\in\Phi}{\min} L(\boldsymbol{x},\boldsymbol{\alpha},\boldsymbol{\beta}) \tag{D-2.3.1-1}$$
...
#### D-2.3.2. Optimal values of primal and dual problem
...
$$p^* = \underset{x\in\Phi}{\min} f(x) = \min(\theta_P(x)) \tag{D-2.3.2-1}$$

Define $d^*$ as the optimal value of the dual problem, i.e.:
$$d^* = \underset{\beta_j\geq0}{\max} \theta_D(\alpha,\beta) \tag{D-2.3.2-2}$$
...
#### D-2.3.3. Relation between $d^*$ and $p^*$
...
<div  align="center">
  <img src=./support_vector_machine_asset/zhihu_wrong_mimmax_deduction.jpeg style = "zoom:20%">
</div>
...
### D-2.4. Lagrangian multiplier and KKT condition basics
...
#### D-2.4.0. Relation between KKT and primal-dual problem (To Be Refined)
...
#### D-2.1.2 Lagrange multiplier method
...
##### D-2.1.2.1 Geometric understanding on equality constrained optimization 
...
##### D-2.1.2.2 Lagrangian Function for equality constrained optimization
...
##### D-2.1.3.1 Geometric understanding and Lagrangian function for inequality constrained optimization
...
\end{aligned}\tag{D-2.1.3.1-1}
$$
There two cases for the position of optimal solution $x^*$, as:

<div  align="center">
  <img src=./support_vector_machine_asset/inequality_constrained_optimization_figure.jpeg style = "zoom:30%">
</div>
